...
def task(work, id):
    ...
    for pvname in work['payload']:
        ...
        if dbr and data and len(data) and 'data' in data[0] and len(data[0]['data']):
            ...
            print('pvname', pvname, 'NOT HANDLED!! DBR:', len(dbr), 'DATA:', data)
    ...
    work['result'] = pa.table([pvnames, timestamps, integers, floats, strings, binaries], schema=schema)
...
writer = parquet.ParquetWriter('demo3.parquet', schema=schema, version='2.6')
